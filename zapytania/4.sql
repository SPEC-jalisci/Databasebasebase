...
HAVING STRING_AGG(product_category.value,',') LIKE '%hardware%'
		AND (STRING_AGG(product_category.value,',') LIKE '%licencja_3%'
		OR STRING_AGG(product_category.value,',') LIKE '%licencja_rok%')